...
**Changing the destination**: `POST /api/rides/:id/dropoff` (alias `/api/tasks/:id/dropoff`) lets the **requester** change where the job ends, or add/remove calling points, right up until it is finished — including mid-trip. The pickup endpoint deliberately freezes the fare (a rider walking to a legal kerb must not reprice a job); a destination change is DIFFERENT WORK, so it re-prices — on the terms already agreed (`ride.option`'s class multiplier × the surge the rider accepted at booking, stored as `ride.surgeMultiplier`, never today's live figure), carrying any accrued `ride.waiting` sats across untouched. `preview: true` prices the change WITHOUT applying it, so the rider taps a second time on a number they have seen. The committed provider gets a `dropoff_updated` WS frame (new fare, previous fare, new route) plus a push. `TaskManager.updateDropoff` persists + re-snapshots.
...
**Booking for someone else**: `passenger` (`{name, note}`) on request. Participant-gated exactly like the pickup note — capped, in memory, absent from every pre-accept payload and from the kind 30078 snapshot. The driver sees a name to call out at the kerb and nothing else about them.
