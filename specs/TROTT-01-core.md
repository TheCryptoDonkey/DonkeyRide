...
This specification defines the **core task lifecycle and state machine** for TROTT (Trust-minimised Real-world Operational Task Transactions) — an open protocol standard for coordinating physical services between strangers over Nostr. TROTT-01 is the foundation specification. Every TROTT implementation MUST implement this spec.

TROTT-01 defines eight parameterised replaceable event kinds (30500-30507) covering the complete lifecycle of a task from request through completion or cancellation. It specifies a universal state machine with domain extension points, enabling any service domain (ridesharing, locksmith dispatch, parcel delivery, towing, pet services, security guard dispatch, and more) to operate on a common protocol backbone.

## Motivation

Traditional service platforms use proprietary APIs and closed data formats, creating vendor lock-in for both requesters and providers. TROTT-01 defines a **common task lifecycle** on Nostr that enables:
...
- **Domain extensibility** — New service domains are added by extending the core state machine, not by forking the protocol
...
Domain extension specs (e.g. TROTT-ridesharing) MAY define aliases for these terms. Implementations SHOULD accept both the generic and domain-specific forms interchangeably.

---

## Event Kinds

All event kinds in this specification are **parameterised replaceable** (NIP-33). Each event MUST include a `d` tag containing the task identifier. Relays replace earlier versions of the same event (same `pubkey` + `kind` + `d` tag) with the latest version.
...
Domain extensions MAY define aliases for location tags (e.g. `pickup_lat` for ridesharing, `lockout_lat` for locksmith). Implementations MUST accept both the generic and aliased forms.
...
All TROTT domains share the following core state machine. States are divided into the **quoting phase** (optional), the **core flow** (mandatory), and **branch states** (reachable from multiple points).
...
Domains insert sub-states **within** the `in_progress` phase. The core protocol treats the entire sub-state sequence as `in_progress`; domain-aware clients render the sub-states for richer UX.
...
The quoting phase is **optional**. Domains that use flat-rate or negotiated pricing (locksmith, emergency trades) typically enable quoting. Domains with algorithmic pricing (ridesharing) typically skip it.
...
**Conditionally required**: `location_lat`, `location_lon` (REQUIRED for geohash-discovered services, OPTIONAL for virtual or category-discovered services)

**Optional tags**: `domain`, `destination_lat`, `destination_lon`, `g`, `amount`, `currency`, `trust_model`, `expiration`, `scheduled_start`, `recurrence`, `recurrence_end`
...
Published by a provider to say "I can do this." Optionally includes a quote. Multiple providers MAY publish offers for the same task, enabling the requester to choose.
...
2. **Provider accepts directly** — The provider publishes a Task Accept referencing the Task Request (30500), skipping the quoting phase
...
**Optional tags**: `domain`, `p`, `requester_pubkey`, `provider_pubkey`, `amount`, `currency`, `trust_model`, `estimated_arrival`, `expiration`
...
Some tasks require multiple providers (e.g. a furniture removal requiring two workers). Multi-provider acceptance is indicated by multiple `p` tags on the Task Accept, each with a confirmation status:
...
The third element of each `p` tag indicates the provider's confirmation status: `confirmed` or `pending`. The task transitions to `in_progress` only when all providers are `confirmed`.

### Kind 30503: Task Update

Published by the provider or operator to signal a state transition between `accepted` and `completed`. This is the primary mechanism for domain-specific sub-state transitions within `in_progress`.
...
**Optional tags**: `domain`, `e`, `p`, `provider_pubkey`, `provider_lat`, `provider_lon`, `eta_seconds`, `distance_remaining_metres`, `expiration`
...
Published by the provider to declare that work is finished. MUST include completion proof as defined by the domain profile.
...
**Optional tags**: `domain`, `e`, `p`, `provider_pubkey`, `proof_data`, `final_amount`, `currency`, `trust_model`, `distance_metres`, `duration_seconds`, `expiration`
...
Published by the requester to confirm that the work has been completed satisfactorily. This event triggers payment release (via the payment and stakes layers defined in TROTT-03 and TROTT-04).
...
Operators MAY implement auto-confirmation with a configurable timeout. If the requester does not publish a Task Confirm (30505) or Task Dispute (30507) within the timeout period (e.g. 24 hours), the operator MAY publish a Task Confirm on behalf of the requester. The auto-confirmation timeout SHOULD be declared in the operator's bond event.
...
- The `pubkey` of the event MUST match the party indicated by `cancelled_by` (or be the operator's pubkey if `cancelled_by` is `operator`)
...
Published by either party to escalate a task to dispute resolution. See TROTT-05 for the full dispute resolution protocol.
...
5. **Declare optional state skipping** — Indicate which sub-states may be skipped (e.g. virtual services skip transit states)
...
```json
["linked_task", "<original_task_id>", "<relationship>"]
```
...
The event kinds 30500-30507 defined in this specification are the canonical TROTT lifecycle events. Implementations transitioning from a prior event schema SHOULD:
